...
namespace U1W
{
    ...
    public class LineMeshUI : Graphic, ILineGenerator2d
    {
        ...
        //private ReadOnlyCollection<Vector2> positionsReadOnly;
        //public ReadOnlyCollection<Vector2> Positions => positionsReadOnly is null ? positionsReadOnly = positions.AsReadOnly() : positionsReadOnly;
        ...
        private void ApplyMeshInfo(VertexHelper vh)
        {
            for (int i = 0; i < verticesPosition.Count; i++)
            {
                vh.AddVert(verticesPosition[i], color, Vector2.zero);
            }
